...
| 文件 | 说明 |
|------|------|
| `agent_orchestrator.py` | AgentNode类 + AgentOrchestrator类 + AgentRole枚举 + AGENT_TEMPLATES模板 + 数据库持久化 + 胶囊加载 |
| `__init__.py` | 模块导出 |
...
下一局开始前（开局加载胶囊）：
  1. load_local_capsules → 从数据库加载历史胶囊（本地实现）
  2. get_capsule_context_prompt → 胶囊格式化为经验提示
  3. 经验提示注入 system prompt → 基于历史经验调整本局行为

进化循环：
  1. 本局表现 → 2. 局后自评 → 3. 改写宪章 → 4. 下局开局加载经验
```

## 当前需求
- [ ] 实际注册 Agent 到 EvoMap（测试 hello 端点）
- [x] 实现局后自评的 LLM 生成（当前自评摘要是硬编码的，应该让 LLM 生成）
- [x] 实现 constitution 自动改写逻辑（基于 Memory recall 的经验，LLM 生成新 constitution）
- [x] 添加 Agent 状态持久化（当前只在内存中，重启丢失）
- [x] 实现 Agent 之间的信息隔离路由（SafeActor 过滤在 session/message 中）
- [x] 开局加载胶囊（create_game_session 自动加载历史胶囊作为经验参考）

## 进度
- ✅ AgentNode 类（注册/心跳/进化/记忆 + 数据库持久化 + 本地胶囊加载 load_local_capsules）
- ✅ AgentOrchestrator 类（批量注册/Session/广播/定向/自评 + load_all_agent_capsules）
- ✅ AgentRole 枚举 + AGENT_TEMPLATES 模板
- ✅ 基础进化框架（Memory record + constitution 更新 + LLM 自评反思 + 局后自评 + 自动改写宪章 + 本地持久化）
- ✅ 开局胶囊加载（create_game_session 自动加载历史经验）
- ✅ 胶囊上下文格式化（get_capsule_context_prompt 生成经验提示）
